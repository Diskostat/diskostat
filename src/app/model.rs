...
use crate::components::CurrentDirectoryList;
...
impl Model {
    pub fn view(&mut self) {
        assert!(self
            ...
            .draw(|f| {
                let chunks = Layout::default()
                    ...
                    .constraints(
                        [
                            ...
                            Constraint::Length(5), // Files list
                        ]
                        .as_ref(),
                    )
                    .split(f.size());
                ...
                self.app.view(&Id::CurrentDirectoryList, f, chunks[4]);
            })
            .is_ok());
    }
    ...
    fn init_app() -> Application<Id, Msg, NoUserEvent> {
        ...
        assert!(app
            .mount(
                Id::CurrentDirectoryList,
                Box::new(CurrentDirectoryList::new(0)),
                Vec::default()
            )
            .is_ok());
        app
        ...
        if let Some(msg) = msg {
            ...
            match msg {
                ...
                Msg::DigitCounterBlur => {
                    ...
                    assert!(self.app.active(&Id::CurrentDirectoryList).is_ok());
                    None
                }
