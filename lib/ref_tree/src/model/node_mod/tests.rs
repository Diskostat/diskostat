...
#[test]
fn test_node_get_children_data() {
    let mut node = Node::new(0);
    let child_arc = node.create_and_attach_child(1);
    let children_data = node.get_children_data();
    assert_eq!(children_data.len(), 1);

    // (data, index)
    assert_eq!(children_data[0].1, 0);
    assert_eq!(child_arc.read().unwrap().data, children_data[0].0);
}
